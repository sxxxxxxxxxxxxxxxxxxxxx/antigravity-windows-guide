...
**📌 为什么必须要单独配置网络？**
Antigravity IDE 和 Antigravity 2.0 的底层网络请求都不会完整遵循 Windows 的系统代理规则（常规代理软件无法稳定接管代码与认证流量）。如果不开启 Tun 模式或配置分流，软件就会直接弹出上面的 `There was an unexpected issue setting up your account`，或出现发消息一直转圈、模型无法加载等问题。

请在以下三种方案中选择一种（==强烈建议直接使用「首选方案：Tun 模式」==）：
...
### Q3：蓝色 `Retry` / Agent execution terminated due to error / HTTP 400 {#faq-chat-400}
...
2. **换节点后彻底重启客户端**：更换纯净节点后，**务必彻底退出 Antigravity（包含任务管理器中的后台进程）** 再重新打开软件，重新发起对话测试。
3. **保持节点固定**：请固定使用长期稳定的优质节点，切忌在对话过程中频繁切换跨国 IP，避免再次被系统判定风控。
4. 若换多个纯净节点仍报 `User location is not supported`，请再检查 **Google 账号关联地区**（详见 [Q0.5 账号没资格](#faq-auth-ineligible)）。
